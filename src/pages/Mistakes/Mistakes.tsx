...
function Mistake() {
  ...
  useEffect(() => {
    const fetchMistakes = async () => {
      ...
      try {
        ...
        const userMistakes = response.data.filter(
          (mistake) => String(mistake.userId) === String(profile.id)
        );
        setMistakes(userMistakes);

        const validMistakes = userMistakes.filter(
          (mistake) =>
        ...
        const [answersResults, questionsResults, lessonsResults] =
          await Promise.all([
            Promise.all(
              answersPromises.map((p) => p.catch((error) => ({ error })))
            ),
            Promise.all(
              questionsPromises.map((p) => p.catch((error) => ({ error })))
            ),
            Promise.all(
              lessonsPromises.map((p) => p.catch((error) => ({ error })))
            ),
          ]);
        ...
        const questionsMap = validMistakes.reduce<
          Record<number, QuestionResponse>
        >((acc, mistake, index) => {
        ...
        const lessonsMap = validMistakes.reduce<Record<number, LessonResponse>>(
          (acc, mistake, index) => {
  ...
  const handleReviewToggle = async (mistake: MistakeResponse) => {
    ...
    if (!mistake.active) {
      try {
        ...
        setMistakes((prev) =>
          prev.map((m) => (m.id === mistake.id ? { ...m, active: true } : m))
        );
  ...
  const currentMistakes = mistakes.slice(
    indexOfFirstMistake,
    indexOfLastMistake
  );
            ...
            <tbody>
              {currentMistakes.map((mistake) => {
                const questionContent =
                  questions[mistake.questionId]?.content ||
                  "Question not found";
                const lesson = lessons[mistake.lessonId];
                // const correctAnswerList = correctAnswers[mistake.questionId]
                //     ?.map(ans => ans.content)
                //     .join(', ') || 'No correct answers found';
                const dateTaken = mistake.updatedAt
                  ? new Date(mistake.updatedAt).toLocaleDateString()
                  : "Invalid Date";
                const correctAnswer =
                  questions[mistake.questionId].correctAnswer;
                ...
                return (
                  <tr key={mistake.id}>
                    ...
                    <td>
                      <button
                        className={
                          mistake.active ? "btn-reviewed" : "btn-not-reviewed"
                        }
