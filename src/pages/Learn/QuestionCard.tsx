...
const QuestionCard: React.FC<QuestionCardProps> = ({ question, onNavigation }) => {
  ...
  return (
    <Card className="text-center my-4">
      <Card.Header>
        <h2>Question {question.id}: Fill in the blank</h2>
      </Card.Header>
      <Card.Body>
        <Card.Text
            style={{
        ...
        <ListGroup className="my-3" style={{ maxWidth: "600px", margin: "auto" }}>
          {answers.map((answer) => (
            <ListGroup.Item
                    key={answer.id}
                    ...
                    style={{
                      backgroundColor: selectedAnswer === answer.id ? "#ffd3b8" : "#f9f9f9", // Màu nền
                    ...
                    onMouseOver={(e) =>
                      (e.currentTarget.style.backgroundColor = selectedAnswer === answer.id
        ...
        {submissionResult && (
          <Alert
            variant={submissionResult === "Correct answer!" ? "success" : "danger"}
            className="mt-3"
          >
            {submissionResult}
          </Alert>
