...
export const createMistake = async (
  ...
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Failed to create mistake"
    );
  }
};
...
export const updateMistake = async (
  ...
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Failed to update mistake"
    );
  }
};

// Fetch all mistakes
export const getAllMistakes = async (): Promise<
  ResponseObject<MistakeResponse[]>
> => {
  try {
    ...
    console.error(
      "Error fetching all mistakes:",
      error.response ? error.response.data : error
    );
    throw new Error(
      error.response?.data?.message ||
        "An error occurred while fetching mistakes"
    );
  }
};
...
export const deleteMistake = async (
  ...
  try {
    ...
    throw new Error(
      error.response?.data?.message || "Failed to delete mistake"
    );
  }
};
